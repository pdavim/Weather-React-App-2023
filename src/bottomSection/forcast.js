import React from "react";
//material ui import
import Grid from "@material-ui/core/Grid";

//import other componets

import ForecastCaroussel from "./forecastCaroussel";

export default class Forcastday extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      n: 0
    };
  }

  //Test if number is even or odd & accept zero has even
  a = n => {
    if (n % 2 === 0) {
      return console.log("even");
    } else {
      return console.log("odd");
    }
  };

  render() {
    const { data } = this.props;
    const dataDay1 = data[8];
    const dataDay2 = data[16];
    const dataDay3 = data[24];
    const dataDay4 = data[32];

    //GET ICONS
    let iconDay1 = `http://openweathermap.org/img/wn/${
      dataDay1.weather[0].icon
    }@2x.png`;
    let iconDay2 = `http://openweathermap.org/img/wn/${
      dataDay2.weather[0].icon
    }@2x.png`;
    let iconDay3 = `http://openweathermap.org/img/wn/${
      dataDay3.weather[0].icon
    }@2x.png`;
    let iconDay4 = `http://openweathermap.org/img/wn/${
      dataDay4.weather[0].icon
    }@2x.png`;

    //Get Date
    let date1 = new Date(dataDay1.dt * 1000);
    let date2 = new Date(dataDay2.dt * 1000);
    let date3 = new Date(dataDay3.dt * 1000);
    let date4 = new Date(dataDay4.dt * 1000);

    var dateData1 = date1.getDate() + "/" + (date1.getMonth() + 1);
    var dateData2 = date2.getDate() + "/" + (date2.getMonth() + 1);
    var dateData3 = date3.getDate() + "/" + (date3.getMonth() + 1);
    var dateData4 = date4.getDate() + "/" + (date4.getMonth() + 1);

    return (
      <div>
        <Grid container spacing={4}>
          <Grid item xs>
            <ForecastCaroussel
              props={data}
              temp1={dataDay1.main.temp}
              temp2={dataDay2.main.temp}
              temp3={dataDay3.main.temp}
              temp4={dataDay4.main.temp}
              icon1={iconDay1}
              icon2={iconDay2}
              icon3={iconDay3}
              icon4={iconDay4}
              date1={dateData1}
              date2={dateData2}
              date3={dateData3}
              date4={dateData4}
              feels1={dataDay1.main.feels_like}
              feels2={dataDay2.main.feels_like}
              feels3={dataDay3.main.feels_like}
              feels4={dataDay4.main.feels_like}
              humi1={dataDay1.main.humidity}
              humi2={dataDay2.main.humidity}
              humi3={dataDay3.main.humidity}
              humi4={dataDay4.main.humidity}
              description1={dataDay1.weather[0].description}
              description2={dataDay2.weather[0].description}
              description3={dataDay3.weather[0].description}
              description4={dataDay4.weather[0].description}
              dt1={dataDay1.dt_txt}
              dt2={dataDay2.dt_txt}
              dt3={dataDay3.dt_txt}
              dt4={dataDay4.dt_txt}
            />
          </Grid>
        </Grid>
      </div>
    );
  }
}
